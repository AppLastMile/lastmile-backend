...
export function createTrackingSocket({ backendHost, token }: TrackingSocketOptions): Socket {
...
export async function startLocationWatch(onPoint: (p: {
  lat: number;
  lng: number;
  speed?: number;
  heading?: number;
...
```ts
function emitLocationUpdate(socket: Socket, shipmentId: number, point: {
  lat: number;
  lng: number;
  speed?: number;
  heading?: number;
