...
describe('TokenService', () => {
  ...
  describe('verify', () => {
    ...
    it('throws when userId is not a valid integer', () => {
      ...
      const body = Buffer.from(JSON.stringify({ sub: 'not-a-number', role: 'donor', exp: Math.floor(Date.now() / 1000) + 3600 }))
        .toString('base64')
        .replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const { createHmac } = require('crypto');
      const sig = createHmac('sha256', 'test-secret').update(`${header}.${body}`).digest()
    ...
    it('throws when token is expired', () => {
      ...
      const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' }))
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const body = Buffer.from(JSON.stringify({ sub: 1, role: 'donor', exp: Math.floor(Date.now() / 1000) - 1 }))
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const { createHmac } = require('crypto');
      const sig = createHmac('sha256', 'test-secret').update(`${header}.${body}`).digest()
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
    ...
    it('throws on invalid base64 payload', () => {
      ...
      const { createHmac } = require('crypto');
      const sig = createHmac('sha256', 'test-secret').update(`${header}.${badBody}`).digest()
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
