...
const makeEvent = (overrides = {}) => ({
  id: 1, name: 'Flood Relief', disasterType: 'flood', city: 'Bogota',
  description: 'desc', date: new Date('2025-01-01'), createdBy: 1, createdAt: new Date(), ...overrides,
});
...
const makeService = (overrides: any = {}) => {
  ...
  return { svc: new EventsService(repo as any, eventEmitter as any), repo, eventEmitter, qb };
};
...
describe('EventsService', () => {
  describe('create', () => {
    it('creates event and emits event.created', async () => {
      ...
      const dto = { name: 'Flood', disasterType: 'flood', city: 'Bogota', description: 'desc', date: '2025-01-01', createdBy: 1 } as any;
  ...
  describe('findAll', () => {
    it('returns paginated events with defaults', async () => {
      const { svc } = makeService({ qb: { getManyAndCount: jest.fn().mockResolvedValue([[makeEvent()], 1]) } });
    ...
    it('applies disasterType filter', async () => {
      ...
      expect(qb.andWhere).toHaveBeenCalledWith(expect.stringContaining('disasterType'), expect.any(Object));
    });
  ...
  describe('update', () => {
    it('updates event fields', async () => {
      const { svc } = makeService();
      const result = await svc.update(1, { name: 'Updated', date: '2025-06-01' } as any);
      expect(result).toBeDefined();
    });
