...
const makeNotification = (overrides = {}) => ({
  id: 1, userId: 1, message: 'test notification', auctionId: null, read: false, createdAt: new Date(), ...overrides,
});
...
const makeService = () => {
  ...
  const campaignsRepo = { findOne: jest.fn().mockResolvedValue({ id: 1, name: 'Camp' }) };
  const messagesRepo = { findOne: jest.fn().mockResolvedValue({ id: 1, campaignId: 1, userId: 2 }) };
  const usersRepo = {
  ...
  const svc = new NotificationsService(
    notifRepo as any, bidsRepo as any, campaignsRepo as any,
    messagesRepo as any, usersRepo as any, dataSource as any, eventEmitter as any,
  );
  return { svc, notifRepo, bidsRepo, campaignsRepo, messagesRepo, usersRepo, eventEmitter, notifQb };
};
...
describe('NotificationsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('applies userId filter', async () => {
      ...
      expect(notifQb.andWhere).toHaveBeenCalledWith(expect.stringContaining('userId'), expect.any(Object));
    });

    it('applies read filter', async () => {
      const { svc, notifQb } = makeService();
      await svc.findAll({ read: false } as any);
      expect(notifQb.andWhere).toHaveBeenCalledWith(expect.stringContaining('read'), expect.any(Object));
    });
  });
  ...
  describe('onAuctionClosed', () => {
    it('creates winner notification', async () => {
      const { svc, eventEmitter } = makeService();
      await svc.onAuctionClosed({ auctionId: 1, winnerId: 2, itemName: 'Laptop', winningAmount: 500, currency: 'USD', closedAt: new Date() });
      expect(eventEmitter.emit).toHaveBeenCalled();
    });

    it('does nothing when no winner', async () => {
      const { svc, eventEmitter } = makeService();
      await svc.onAuctionClosed({ auctionId: 1, winnerId: null, itemName: 'Laptop', winningAmount: 0, currency: 'USD', closedAt: new Date() });
  ...
  describe('onAuctionSold', () => {
    it('notifies buyer and losing bidders', async () => {
      ...
      bidsRepo.find.mockResolvedValue([{ userId: 3, auctionId: 1 }, { userId: 4, auctionId: 1 }]);
      await svc.onAuctionSold({ auctionId: 1, buyerId: 3, price: 100, currency: 'USD', soldAt: new Date(), itemName: 'Laptop' } as any);
      expect(eventEmitter.emit).toHaveBeenCalled();
    });
  });
});
