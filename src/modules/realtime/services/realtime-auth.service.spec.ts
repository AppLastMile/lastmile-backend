...
const makeValidToken = (payload: object, secret = 'test-secret') => {
  const tokenService = new TokenService({ get: jest.fn().mockReturnValue(secret) } as unknown as ConfigService);
...
describe('RealtimeAuthService', () => {
  describe('verifyToken', () => {
    ...
    it('throws on expired token', () => {
      ...
      const body = Buffer.from(JSON.stringify({ sub: 1, role: 'donor', exp: Math.floor(Date.now() / 1000) - 100 }))
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const sig = createHmac('sha256', secret).update(`${header}.${body}`).digest()
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const svc = makeService(secret);
      expect(() => svc.verifyToken(`${header}.${body}.${sig}`)).toThrow(UnauthorizedException);
    });
    ...
    it('throws when userId is invalid', () => {
      ...
      const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' }))
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const body = Buffer.from(JSON.stringify({ sub: 'not-an-int', role: 'donor' }))
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const sig = createHmac('sha256', secret).update(`${header}.${body}`).digest()
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const svc = makeService(secret);
    ...
    it('throws on invalid payload base64', () => {
      ...
      const sig = createHmac('sha256', secret).update(`${header}.${badBody}`).digest()
        .toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const svc = makeService(secret);
      expect(() => svc.verifyToken(`${header}.${badBody}.${sig}`)).toThrow(UnauthorizedException);
    });
    ...
    it('uses fallback secret when config returns undefined', () => {
      ...
      const tokenSvc = new TokenService({ get: jest.fn().mockReturnValue('dev-jwt-secret') } as unknown as ConfigService);
